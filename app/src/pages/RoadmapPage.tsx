            ...
            <div className="flex flex-col items-center gap-6 mb-10 text-center">
                ...
                <div className="space-y-4 w-full">
                    <div className="flex items-center justify-center gap-3">
                        <span className="px-2.5 py-1 rounded-md bg-primary/10 text-primary text-[10px] font-black uppercase tracking-[0.25em]">{data.title}</span>
                        <ChevronRight size={12} className="text-muted-foreground/30" />
                        <span className="text-[10px] font-black uppercase tracking-[0.25em] text-muted-foreground">Статья {currentSectionIdx as number + 1}</span>
                    </div>
                    <h1 className="text-4xl lg:text-5xl font-black tracking-tight text-foreground leading-tight max-w-3xl mx-auto">{section.title}</h1>
                </div>
            </div>

            <div className="glass rounded-[2.5rem] p-3 lg:p-4 transition-all border border-white/5">
                <div className="rounded-[2.2rem] bg-background/40 p-6 lg:p-10 space-y-12 shadow-inner shadow-white/5 overflow-hidden">
                    {/* Image */}
                    {section.image && (
                        <div className="group relative overflow-hidden rounded-[2rem] flex justify-center items-center">
                    ...
                    <div className="space-y-12">
                        <div className="grid gap-8">
                            {section.content.map((item: string, i: number) => {
                                ...
                                if (isHeader) {
                                    return (
                                        <h4 key={i} className="text-2xl font-black text-foreground mt-16 mb-6 tracking-tight flex items-center gap-4 group border-l-4 border-primary pl-6">
                                            <span className="flex-1 leading-tight">{trimmed}</span>
                                        </h4>
                                    );
                                }
                                ...
                                return (
                                    <div key={i} className={`relative group/line ${isListItem ? 'ml-6' : ''}`}>
                                        <div className="absolute -inset-y-3 -inset-x-5 bg-primary/5 rounded-2xl scale-95 opacity-0 group-hover/line:scale-100 group-hover/line:opacity-100 transition-all duration-300 -z-10" />
                                        <div className="flex gap-4 font-medium leading-[1.8] text-foreground/90 text-[1.1rem]">
                                            {isListItem ? (
                        ...
                        {section.glossary && section.glossary.length > 0 && (
                            <div className="p-8 lg:p-10 rounded-[2rem] bg-primary/5 border border-primary/10 relative overflow-hidden group/glossary mt-12">
                                <div className="absolute top-0 right-0 p-10 h-60 w-60 bg-primary/10 rounded-full blur-[80px] -mr-32 -mt-32 group-hover/glossary:scale-110 transition-transform duration-1000" />
                                <div className="flex items-center gap-3 text-primary font-black text-xs uppercase tracking-[0.3em] mb-8 relative z-10">
                                    <BookOpen size={20} /> Словарь терминов
                                </div>
                                <div className="grid gap-6 relative z-10">
                                    {section.glossary.map((term: any, i: number) => (
                                        <div key={i} className="space-y-1">
                                            <span className="text-lg font-black text-foreground block">{term.term}</span>
                                            <p className="text-foreground/70 font-medium leading-relaxed">{term.definition}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {section.mistakes && section.mistakes.length > 0 && (
                            <div className="p-8 lg:p-10 rounded-[2rem] bg-red-500/5 border border-red-500/20 relative overflow-hidden group/mistake">
                                <div className="absolute top-0 right-0 p-10 h-60 w-60 bg-red-500/10 rounded-full blur-[80px] -mr-32 -mt-32 group-hover/mistake:scale-110 transition-transform duration-1000" />
                                <div className="flex items-center gap-3 text-red-500 font-black text-xs uppercase tracking-[0.3em] mb-8 relative z-10">
                                    <XCircle size={20} /> Типичные ошибки новичков
                                </div>
                                <div className="grid gap-6 relative z-10 text-foreground">
                                    {section.mistakes.map((mistake: string, i: number) => (
                                        <div key={i} className="p-5 rounded-2xl bg-red-500/10 border border-red-500/10 font-bold leading-relaxed flex gap-4 italic shadow-sm">
                        ...
                        {section.quiz && section.quiz.length > 0 && (
                            <div className="space-y-8 pt-10 border-t border-white/5">
                                <div className="flex items-center gap-3 text-[10px] font-black text-foreground/40 uppercase tracking-[0.4em] mb-4">
                                    ...
                                    {section.quiz.map((q: any, qIdx: number) => {
                                        ...
                                        return (
                                            <div key={qIdx} className="space-y-6">
                                                <h5 className="text-xl font-black text-foreground pr-8 leading-snug">{qIdx + 1}. {q.question}</h5>
                                                    ...
                                                    {q.options.map((option: string, oIdx: number) => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                className={`w-full text-left p-5 rounded-2xl border transition-all flex items-center justify-between gap-4 font-bold text-[15px] ${bgClass} ${!isAnswered ? 'hover:scale-[1.01] active:scale-[0.99] cursor-pointer' : 'cursor-default'}`}
                                                            >
                                                                <div className="flex items-center gap-4">
                                                                    <span className={`h-8 w-8 shrink-0 rounded-xl flex items-center justify-center text-xs font-black shadow-inner ${isSelected ? 'bg-foreground text-background' : 'bg-background/50 text-foreground/60'}`}>
                                                ...
                                                {isAnswered && (
                                                    <div className={`text-sm font-black p-4 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-left-4 duration-500 ${selectedIdx === q.correctAnswer ? 'text-emerald-400/80 bg-emerald-500/5' : 'text-red-400/80 bg-red-500/5'}`}>
